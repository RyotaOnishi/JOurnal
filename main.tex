...
\author{Ryota~Onishi${}^{1}$,
        Akira~Furui${}^{1}$,~\IEEEmembership{Member,~IEEE,}
        Akihito~Takeuchi${}^{2}$,
        Tomoyuki~Akiyama${}^{3}$,
...
\begin{abstract}
\textit{Objective:}
The Detection of epileptic seizures in surface electroencephalogram (EEG) signals is an important probrem in biomedical engineering.
In this paper, we proposed analysis method for multichannel EEG signals using statistical properties, and show the validity to model parameter applied to the detection of epileptic seizures.
\textit{Methods:}
This paper proposes a surface EEG analysis method on the basis of a multivariate scale mixture model (MVSMM) that can be evaluated Gaussianity and non-Gaussianity by one dimensional parameter.
...
\textit{Results:}
We applied the proposed method to the multichannel EEG data from twenty patients with focal epilepsy. The results demonstrated that the estimated parameter can be evaluated seizures or non-seizures in high frequency band. It was also shown that the proposed method provides a higher classification performance than conventional method by using ROC analysis (AUC = 0.881).
\textit{Conclusion:}
...
In high frequency band, estimated parameter shows the possibility of clasification of epileptic seizures.
\textit{Significance:}
...
\begin{figure}[!t] %20名
\centering
\includegraphics[width=0.9\hsize]{figure/ROC_rms_ver3.eps}
%\end{center}
