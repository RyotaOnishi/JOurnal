...
\end{eqnarray}
ここで，$\mathrm{ln}L(\hat{\theta})$はモデルの尤度，$p$はモデルで推定されるパラメータ数，$N_W$は移動窓内のサンプル数である．
...
\begin{table}[t]
 \begin{center}
 \caption[Subject conditions]{Percentage of the times each model was selected for different window widths.}
 \label{table:conditions}
 \vspace{-2.5mm}
% \scalebox{0.86}[0.86]{%size
  \begin{tabular}{cccc}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.9\hsize]{figure/AAPE_color_dim.eps}
%\end{center}
